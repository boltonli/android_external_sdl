    ...
    const JNINativeMethod* gMethods, int numMethods)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Registering %s natives\n", className);
...
int jniThrowException(JNIEnv* env, const char* className, const char* msg)
{
    ...
    if ((*env)->ExceptionCheck(env)) {
        ...
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Discarding pending exception (%s) to throw %s\n",
            buf, className);
    }

    exceptionClass = (*env)->FindClass(env, className);
    if (exceptionClass == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Unable to find exception class %s\n", className);
        /* ClassNotFoundException now pending */
        return -1;
    }

    if ((*env)->ThrowNew(env, exceptionClass, msg) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Failed throwing '%s' '%s'\n", className, msg);
