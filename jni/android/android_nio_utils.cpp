...
#define NULL 0
#define LOG_TAG "android_nio_utils.cpp"
...
static jclass findClass(JNIEnv* env, const char name[]) {
    ...
    if(c == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Unable to find class %s", name);
    }
    return c;
}
...
static jmethodID findStaticMethod(JNIEnv* env, jclass c, const char method[],
    ...
    if(m == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Unable to find method %s", method);
    }
    return m;
}
...
static jfieldID getFieldID(JNIEnv* env, jclass c, const char name[],
    ...
    if(f == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Unable to find field %s", name);
    }
    return f;
}
