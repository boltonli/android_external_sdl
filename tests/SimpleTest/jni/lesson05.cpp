								 ...
								 JNINativeMethod* gMethods, int numMethods)
{
    ...
    if (clazz == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Native registration unable to find class '%s'", className);
        return JNI_FALSE;
    }
    if (env->RegisterNatives(clazz, gMethods, numMethods) < 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "RegisterNatives failed for '%s'", className);
        return JNI_FALSE;
    }
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    if (vm->GetEnv(&uenv.venv, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "ERROR: GetEnv failed");
        goto bail;
    }
    env = uenv.env;
	
    if (registerNatives(env) != JNI_TRUE) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "ERROR: registerNatives failed");
        goto bail;
    }
